...
PRO <- data.table::fread("C:/Users/k16206jp/OneDrive - The University of Manchester/OMD paper joint editing folder/6 OMD Paper/Patient Samples MS Analysis/Proteome/1 Filtering before DEP/Less than 2000 proteins samples removed Min 80 1 group.txt")

names(PRO)[names(PRO) == "Gene names"] <- "Gene.names"
names(PRO)[names(PRO) == "Protein IDs"] <- "Protein.IDs"

#log2 transform data
intensityPRO <- PRO %>% 
  select(grep("LFQ", colnames(.))) 
ColumnTitles <- PRO[,c(45:59)]
...
experimental_design <- data.table::fread("C:/Users/k16206jp/OneDrive - The University of Manchester/OMD paper joint editing folder/6 OMD Paper/Patient Samples MS Analysis/Proteome/2 DEP Processing/experimental design.txt")
...
write.table(filtered.norm.data.limma.test, file="C:/Users/k16206jp/OneDrive - The University of Manchester/OMD paper joint editing folder/6 OMD Paper/Patient Samples MS Analysis/Proteome/2 DEP Processing/filtered normalised proteome pre imputation.txt", sep="\t", quote=F)

#save normalised dataframe post imputation
filtered.norm.data.limma.test.imp <- as.data.frame(assays(data_imp)[[1]])
log.sub.raw.data.matrix <- filtered.norm.data.limma.test.imp
write.table(filtered.norm.data.limma.test.imp, file="C:/Users/k16206jp/OneDrive - The University of Manchester/OMD paper joint editing folder/6 OMD Paper/Figure 1 Patient MS and OMD identification/Bone met vs non met filtered normalised proteome post imputation.txt", sep="\t", quote=F)
filtered.norm.data.limma.test.imp.with.titles <- cbind(ColumnTitles,filtered.norm.data.limma.test.imp)
write.table(filtered.norm.data.limma.test.imp.with.titles, file="C:/Users/k16206jp/OneDrive - The University of Manchester/OMD paper joint editing folder/6 OMD Paper/Bone met vs non met filtered normalised proteome post imputation with titles.txt", sep="\t", quote=F)
...
log.sub.raw.data.matrix<-log.sub.raw.data.matrix[,c(1:10,15,16,18:23,25:34,37:42,44)]
head(log.sub.raw.data.matrix)
m.design <- c("N","N","N","N","N","N","N","N","N","N","Y","Y","Y","Y","Y","Y","Y","Y","Y","N","N","N","N","N","N","N","N","N","Y","Y","Y","Y","Y","Y","Y")
names(log.sub.raw.data.matrix) <- m.design
head(log.sub.raw.data.matrix)
...
cont.matrix <- makeContrasts("Bone Mets vs Non-Mets" =Y - N,
...
#export results
write.table(tT, file="C:/Users/k16206jp/OneDrive - The University of Manchester/OMD paper joint editing folder/6 OMD Paper/Figure 1 Patient MS and OMD identification/Bone met vs non met Adjusted P value Data.txt", sep="\t",quote=F, row.names = F)
