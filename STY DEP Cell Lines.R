...
STY <- data.table::fread("C:/Users/k16206jp/OneDrive - The University of Manchester/OMD paper joint editing folder/6 OMD Paper/OMD Cells MS analysis/Phosphoproteome/STY filtered min 2 1 group.txt")

names(STY)[names(STY) == "Gene names"] <- "Gene.names"
names(STY)[names(STY) == "Proteins"] <- "Protein.IDs"

#log2 transform data
intensitySTY <- STY %>% 
  select(grep("0", colnames(.))) 
ColumnTitles <- STY[,c(13:35)]
...
experimental_design <- data.table::fread("C:/Users/k16206jp/OneDrive - The University of Manchester/OMD paper joint editing folder/6 OMD Paper/OMD Cells MS analysis/Phosphoproteome/DEP analysis/experimental design.txt")
...
write.table(filtered.norm.data.limma.test, file="C:/Users/k16206jp/OneDrive - The University of Manchester/OMD paper joint editing folder/6 OMD Paper/OMD Cells MS analysis/Phosphoproteome/DEP analysis/filtered normalised proteome pre imputation.txt", sep="\t", quote=F)
...
write.table(filtered.norm.data.limma.test.imp, file="C:/Users/k16206jp/OneDrive - The University of Manchester/OMD paper joint editing folder/6 OMD Paper/OMD Cells MS analysis/Phosphoproteome/DEP analysis/filtered normalised proteome post imputation.txt", sep="\t", quote=F)
filtered.norm.data.limma.test.imp.with.titles <- cbind(ColumnTitles,filtered.norm.data.limma.test.imp)
write.table(filtered.norm.data.limma.test.imp.with.titles, file="C:/Users/k16206jp/OneDrive - The University of Manchester/OMD paper joint editing folder/6 OMD Paper/OMD Cells MS analysis/Phosphoproteome/DEP analysis/filtered normalised proteome post imputation with titles.txt", sep="\t", quote=F)
...
PAMPAO <- log.sub.raw.data.matrix[,c(1:6)]
head(PAMPAO)
m.design <- c("N","N","N","Y","Y","Y")
colnames(PAMPAO) <- m.design
head(PAMPAO)
design <- model.matrix(~0+factor(m.design, levels=unique(m.design)))
design
colnames(design) <- unique(m.design)
fit <- lmFit(PAMPAO, design)
cont.matrix <- makeContrasts("PAO vs PAM" =Y - N,
...
#export results
write.table(tT, file="C:/Users/k16206jp/OneDrive - The University of Manchester/OMD paper joint editing folder/6 OMD Paper/OMD Cells MS analysis/Phosphoproteome/DEP analysis/Adjusted P value Data.txt", sep="\t",quote=F, row.names = F)
